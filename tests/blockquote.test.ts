...
test("Blockquote", async ({ expect }) => {
  const largeHTML = await fs.readFile("tests/blockquote.html", "utf8")
  ...
  await writeFile(docxContent, "tests/blockquote_tmp_.docx")
  ...
  expect(docXml).toMatchFileSnapshot("./blockquote.xml")
})
