...
test("font HTML", async ({ expect }) => {
  const largeHTML = await fs.readFile("tests/font.html", "utf8")
  ...
  await writeFile(docxContent, "tests/font_tmp_.docx")
  ...
  expect(docXml).toMatchFileSnapshot("./font.xml")
})
